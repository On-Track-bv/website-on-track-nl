...
// CoachMark component: position absolute, points to profielknop
import React from 'react';
type CoachMarkPos = { top: number; left: number; width: number; height: number };
const CoachMark = ({ lang }: { lang: string }) => {
  const [pos, setPos] = React.useState<CoachMarkPos | null>(null);
  
  React.useEffect(() => {
    // Verberg CoachMark op mobile
    if (window.innerWidth < 600) return;
    const el = document.querySelector('[data-profile-btn]') as HTMLElement | null;
    if (el) {
  ...
  const text = lang === 'nl' ? 'Verander hier je rol!' : 'Change your role here!';
  const color = 'var(--my-primary)';

  // Position the arrow closer to the button (more to the left)
  // Place the text bottom-left of the arrow using absolute positioning
  return (
    <div
      style={{
        position: 'absolute',
        ...
        zIndex: 99999,
        pointerEvents: 'none',
        animation: 'fadeInCoachMark 0.5s',
      }}
    >
...
export function Home() {
  ...
  const handleRoleSelect = (key: string) => {
    ...
    setTimeout(() => setShowCoachMark(true), 400); // Even wachten tot header zichtbaar is
  };

  // Verberg coach mark bij scrollen of klikken
  useEffect(() => {
    if (!showCoachMark) return;
    const hide = () => setShowCoachMark(false);
